...
> **独立开放标准声明**：本文档定义的是 **Story-Repo 文件格式标准**，**不绑定 story-cli 实现**。
> 任何工具（Obsidian 插件、VS Code 扩展、自定义脚本、AI 工作流）只要遵守本章约定的
> 目录结构与字段语义，即可读写符合规范的 story 仓库。
>
> 📋 CLI 命令清单见 [commands.md](commands.md)。
>
> **版本策略**：本文档版本（v2.0）独立于 CLI 版本。
> 新增可选字段时递增 minor 版本（v2.0 → v2.1）；
> 破坏性变更（移除/重命名字段）时递增 major 版本（v2.x → v3.0）。
>
> **非目标**：本规范只约定 `NN-目录` / `config.json` / `text.md` / `chapter-*.md` 的
> 结构与字段语义，**不约束 CLI 命令接口、UI 交互、EPUB 导出细节、README 模板设计**——这些属于各实现的具体行为。
...
| `links`          | string[] | 关联故事文件夹名列表（可选弱关联，指向同仓库内的 `NN-` 目录名） |

> **`links` 弱关联语义**（v2.0 新增）：`links` 声明本故事与其他故事的人工关联（如同系列、共享角色、
> 引申阅读）。它是「弱关联」——只依赖文件路径与元数据，**不依赖图数据库或向量库**；读取器可据此
> 渲染「关联故事」，写入端应保证列表内为同仓库的有效文件夹名并提供去重。（详情见各实现，如 story-cli
> 的 `story link` 命令与 `build` 建议层。）
...
故事正文使用 Markdown 格式，由内置渲染器（`md-to-html.ts`）处理，HTML 导出与 EPUB 导出共用同一套渲染逻辑。
...
| 行内代码   | `` `code` ``          |                                       |
| 标题       | `#` ~ `######`        | `#` / `##` → h2，`###` 及以上 → h3    |
| 引用块     | `> 引用内容`          | 支持引用块内嵌套列表 / 多段落         |
| 链接       | `[文字](https://...)` | 支持 URL 括号配对、中文路径、查询参数 |
| 图片       | `![alt](assets/cover.png)` | 支持相对/绝对路径，用于 EPUB 封面与正文插图 |
| 无序列表   | `- 项目`              | 支持多级缩进嵌套 + 缩进续行           |
| 有序列表   | `1. 项目`             | 支持多级缩进嵌套 + 缩进续行           |
| 表格       | `\| 列1 \| 列2 \|`    | 标准 Markdown 表格，支持对齐分隔符    |
| 代码块     | ` ```js ...``` `      | 保留原始格式，HTML 安全转义           |
| 水平线     | `---` / `***`         |                                       |
| 反斜杠转义 | `\* 不是斜体 \*`      | 支持 `\` + `*`、`_`、`[` 等常见转义   |

#### 明确不支持的语法

| 语法             | 原因                                     |
| ---------------- | ---------------------------------------- |
| 原始 HTML 标签   | 出于安全考虑会被转义为纯文本（XSS 防护） |
| 任务列表 `- [x]` | 未实现                                   |
| 脚注 / 定义列表  | 未实现                                   |
| LaTeX 数学公式   | 未实现                                   |

> 💡 上述语法子集覆盖了小说 / 论文 / 教程等绝大多数创作场景。如有特殊排版需求，建议在 `summary` 或正文中使用普通文本描述。
