...
| `link` |      | `story link <source> <target>`                                                                      | 添加关联；`--remove=<target>` 移除；`--list` 列出全部或指定故事                      | [add-story](add-story.md) |
...
| `epub`              | `e`  | `story epub "标题" [--all] [--split-by-volume]`     | EPUB 3 电子书（支持分卷/封面/图片）；`--all` 导出全部 | [epub](epub.md)     |
...
| `--output=dir`      | `export *`、`import json`             | 自定义输出目录                            |
